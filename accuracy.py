...
def accu(output, target, cuda):
    ...
    length_correct = (length_predict == target[:, 0].contiguous().view(-1, 1)).float()
    _, digit1_predict = torch.max(output[1], 1)
    digit1_correct = (((digit1_predict == target[:, 1].contiguous().view(-1, 1)) + (1>length_predict)*2) > 0).float()     
    digits_correct = digit1_correct #all digits should be predicted correctly
    for i in range(4):
        _, digit_predict = torch.max(output[i+2], 1)
        digit_correct = (((digit_predict == target[:, i+2].contiguous().view(-1, 1)) + ((i+2)>length_predict)*2) > 0).float()
